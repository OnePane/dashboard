...
function App() {
  ...
  const connectionsByPlatform = connections.reduce<Record<string, ApiConnection[]>>((groups, connection) => { (groups[connection.provider] ||= []).push(connection); return groups }, {})
      ...
      <main className="dashboard">
        ...
        {activePage === 'connections' ? <section className="settings-grid">
          <article className="panel connections-panel" id="settings">
            ...
            <div className="platform-list">{connections.length ? Object.entries(connectionsByPlatform).map(([platform, platformConnections]) => { const details = providerDetails[platform] || { mark: platform.slice(0, 1), tone: 'boa' }; return <section className="platform-group" key={platform}><div className="platform-group-heading"><ProviderMark mark={details.mark} tone={details.tone} /><div><strong>{providerOptions.find((option) => option.id === platform)?.name || platform}</strong><small>{platformConnections.length} API {platformConnections.length === 1 ? 'key' : 'keys'}</small></div><button className="quiet-button" type="button" onClick={() => { setEditingConnection(null); setSelectedProvider(platform); setShowProviderFlow(true) }}>Add key</button></div><div className="connection-list">{platformConnections.map((connection) => <div className="connection" key={connection.id}><div className="key-icon">⌁</div><div><strong>{connection.label || 'Unnamed key'}</strong><small>•••• {connection.keyLast4} · Added {new Date(connection.connectedAt).toLocaleString()}</small></div><div className="connection-actions"><button type="button" onClick={() => { setEditingConnection(connection); setSelectedProvider(connection.provider); setShowProviderFlow(true) }}>Edit</button><button type="button" onClick={async () => { if (!window.confirm(`Remove ${connection.label || connection.provider}?`)) return; const response = await apiFetch(`/platform-connections/${connection.id}`, { method: 'DELETE' }); if (!response.ok) { setApiError('The connection could not be removed.'); return } setConnections((current) => current.filter((item) => item.id !== connection.id)); void refreshAccounts() }}>Delete</button></div></div>)}</div></section> }) : <p className="empty-state">No platform connections yet.</p>}</div>
          </article>
