      ...
      <main className="dashboard">
        ...
        {activePage === 'connections' ? <section className="settings-grid">
          <article className="panel connections-panel" id="settings">
            <div className="panel-heading"><div><h2>Platform connections</h2><p>Manage every connected platform and API key</p></div><button className="quiet-button" type="button" onClick={() => setShowProviderFlow(true)}>Add connection</button></div>
            ...
            <div className="connection-list">{connections.length ? connections.map((connection) => { const details = providerDetails[connection.provider] || { mark: connection.provider.slice(0, 1), tone: 'boa' }; return <div className="connection" key={connection.id}><ProviderMark mark={details.mark} tone={details.tone} /><div><strong>{connection.label || connection.provider}</strong><small>•••• {connection.keyLast4} · Connected {new Date(connection.connectedAt).toLocaleString()}</small></div><button type="button" onClick={async () => { if (!window.confirm(`Remove ${connection.label || connection.provider}?`)) return; const response = await apiFetch(`/platform-connections/${connection.id}`, { method: 'DELETE' }); if (!response.ok) { setApiError('The connection could not be removed.'); return } setConnections((current) => current.filter((item) => item.id !== connection.id)); void refreshAccounts() }}>Remove</button></div> }) : <p className="empty-state">No platform connections yet.</p>}</div>
          </article>
