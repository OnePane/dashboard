    ...
    <div className={`app-shell ${activePage === 'connections' ? 'settings-view' : ''}`}>
      <aside className="sidebar">
        ...
        <div className="sidebar-footer">
          <a className={`nav-link ${activePage === 'connections' ? 'active' : ''}`} href="/settings" onClick={(event) => { event.preventDefault(); navigate('connections') }}><i>⚙</i> Settings</a>
...
function SendMoneyModal({ step, setStep, sourceAccounts, sourceAccountId, setSourceAccountId, recipients, selectedRecipient, setSelectedRecipient, amount, setAmount, selectedSource, selectedTransferRecipient, onClose }: { step: number; setStep: (step: number) => void; sourceAccounts: SourceAccount[]; sourceAccountId: string; setSourceAccountId: (id: string) => void; recipients: Recipient[]; selectedRecipient: string; setSelectedRecipient: (id: string) => void; amount: string; setAmount: (amount: string) => void; selectedSource?: SourceAccount; selectedTransferRecipient?: Recipient; onClose: () => void }) {
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}><section className="provider-modal send-money-modal" role="dialog" aria-modal="true" aria-labelledby="send-money-title"><div className="modal-heading"><div><p className="eyebrow">Send money · Step {step} of 4</p><h2 id="send-money-title">{step === 4 ? 'Review transfer' : 'Send money'}</h2></div><button className="modal-close" type="button" aria-label="Close" onClick={onClose}>×</button></div><div className="step-indicator"><span className={step >= 1 ? 'active' : ''} /><span className={step >= 2 ? 'active' : ''} /><span className={step >= 3 ? 'active' : ''} /><span className={step >= 4 ? 'active' : ''} /></div>{step === 1 && <div className="provider-step"><h3>Choose a source account</h3><div className="choice-list">{sourceAccounts.map((account) => <button className={`choice-row ${sourceAccountId === account.id ? 'selected' : ''}`} type="button" key={account.id} onClick={() => setSourceAccountId(account.id)}><span><strong>{account.nickname}</strong><small>{account.provider} · {account.currency} balance {account.balance}</small></span><i>{sourceAccountId === account.id ? '✓' : '›'}</i></button>)}</div><button className="transfer-button" type="button" disabled={!sourceAccountId} onClick={() => setStep(2)}>Continue <span>→</span></button></div>}{step === 2 && <div className="provider-step"><h3>Choose a recipient</h3><div className="choice-list">{recipients.map((recipient) => <button className={`choice-row ${selectedRecipient === recipient.id ? 'selected' : ''}`} type="button" key={recipient.id} onClick={() => setSelectedRecipient(recipient.id)}><span><strong>{recipient.name}</strong><small>{recipient.destination}</small></span><i>{selectedRecipient === recipient.id ? '✓' : '›'}</i></button>)}</div><div className="modal-actions"><button className="modal-back" type="button" onClick={() => setStep(1)}>Back</button><button className="transfer-button" type="button" disabled={!selectedRecipient} onClick={() => setStep(3)}>Continue <span>→</span></button></div></div>}{step === 3 && <form className="provider-step" onSubmit={(event) => { event.preventDefault(); setStep(4) }}><h3>Enter amount</h3><label>Amount<input type="number" name="amount" min="0.01" step="0.01" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="0.00" required /></label><small className="settings-note">Available balance: {selectedSource?.balance || '—'}</small><div className="modal-actions"><button className="modal-back" type="button" onClick={() => setStep(2)}>Back</button><button className="transfer-button" type="submit">Review transfer <span>→</span></button></div></form>}{step === 4 && <div className="provider-step"><h3>{amount ? `$${Number(amount).toFixed(2)}` : '$0.00'} to {selectedTransferRecipient?.name}</h3><div className="review-list"><div><span>From</span><strong>{selectedSource?.nickname} · {selectedSource?.currency}</strong></div><div><span>Recipient</span><strong>{selectedTransferRecipient?.destination}</strong></div></div><button className="transfer-button" type="button" disabled>Send money</button><p className="settings-note">Sending is locked until PayPal/Venmo authorization and money-movement APIs are enabled.</p><div className="modal-actions"><button className="modal-back" type="button" onClick={() => setStep(3)}>Back</button><button className="modal-back" type="button" onClick={onClose}>Cancel</button></div></div>}</section></div>
}
